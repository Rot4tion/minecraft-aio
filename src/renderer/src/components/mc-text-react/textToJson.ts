...
interface TextComponent {
  text: string
  extra: TextFormatting[]
}

const colorMap: { [key: string]: keyof ColorMap } = {
  '0': 'black',
  '1': 'dark_blue',
  '2': 'dark_green',
  '3': 'dark_aqua',
...
export default function convertTextToJson(text: string, prefix: string = '§'): TextComponent {
  const component: TextComponent = { text: '', extra: [] }
  const re = new RegExp(`${prefix}[0-9a-fklmnor]`, 'gm')

  const unformattedText = text.replace(re, '')

  let currentFormattings: TextFormatting = {}
